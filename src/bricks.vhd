...
architecture arch of bricks is
	signal matrix : std_logic_vector(0 to 63) := (others => '0');
	type int_array is array (0 to 3) of integer;
begin
   process (clk, resetN)
		  variable temp_matrix : std_logic_vector(0 to 63);
        variable lfsr : std_logic_vector(15 downto 0);
        variable feedback : std_logic;
        variable brick_count : integer range 0 to 63 := 0;
        variable row_count : int_array := (others => 0);
        variable rand_index : integer range 0 to 63;
    begin
        if resetN = '0' or clrN = '0' then
            ...
            for i in 0 to 31 loop
                if brick_count < 15 then
                    ...
                    if lfsr /= "0000000000000000" then
                        rand_index := to_integer(unsigned(lfsr(5 downto 0))) mod 32;
            ...
            -- Phase 2: Add up to 30 bricks
            for i in 0 to 31 loop
                if brick_count < 30 then
                    feedback := lfsr(15) xor lfsr(13) xor lfsr(12) xor lfsr(10);
                    lfsr := lfsr(14 downto 0) & feedback;
                    if lfsr /= "0000000000000000" then
                        rand_index := to_integer(unsigned(lfsr(5 downto 0))) mod 32;
            ...
            for i in 0 to 31 loop
                if matrix(i) = '1' and
                   ...
                   ball_y + BALL_SIZE >= (i/8) * (BRICK_HEIGHT + 10) + 40 and
                   ball_y <= (i/8) * (BRICK_HEIGHT + 10) + 60 then
                    matrix(i) <= '0';
                    brick_hit <= '1';
                end if;
            end loop;
