...
entity breakout_two_player is
   port ( clk_50              : in  std_logic;     -- 50 MHz clock
          ...
          left1, right1       : in  std_logic;     -- Player 1 controls (bottom paddle)
          left2, right2       : in  std_logic;     -- Player 2 controls (top paddle)
          enable              : in  std_logic;
...
end breakout_two_player;

architecture arch of breakout_two_player is
   -- Game Clock signal
   signal clk_game : std_logic;    -- ~60 Hz for game logic

   -- Game objects
   signal paddle1_x, paddle1_y : unsigned(9 downto 0);  -- Bottom paddle
   signal paddle2_x, paddle2_y : unsigned(9 downto 0);  -- Top paddle
   signal ball_x, ball_y       : unsigned(9 downto 0);
   signal stop                 : std_logic := '1';
   signal brick_matrix         : std_logic_vector(0 to 31);  -- 8x4 grid
   ...
   signal hit_p1, hit_p2       : std_logic;
   ...
   constant BRICK_START_Y : integer := 180;  -- Center bricks vertically
   ...
   component ball_two_player
      ...
      port ( clk          : in  std_logic;
             ...
             clrN         : in  std_logic;
             stop         : in  std_logic;
             paddle1_x    : in  unsigned(9 downto 0);
             paddle1_y    : in  unsigned(9 downto 0);
             paddle2_x    : in  unsigned(9 downto 0);
...
begin
   ...
   u2: paddle_two_player
      generic map (
         PAD_WIDTH    => PAD_WIDTH,
         SCREEN_WIDTH => SCREEN_WIDTH,
         PAD_HEIGHT   => PAD_HEIGHT,
         PADDLE_Y     => SCREEN_HEIGHT - PAD_HEIGHT - 8  -- Bottom paddle at y = 464
      )
      port map (
         clk    => clk_game,
         resetN => resetN,
   ...
   u3: paddle_two_player
      generic map (
         PAD_WIDTH    => PAD_WIDTH,
         SCREEN_WIDTH => SCREEN_WIDTH,
         PAD_HEIGHT   => PAD_HEIGHT,
         PADDLE_Y     => 8  -- Top paddle at y = 8
      )
      port map (
         clk    => clk_game,
         resetN => resetN,
   ...
   pixel_gen: process (count_h, count_v, video, paddle1_x, paddle1_y, paddle2_x, paddle2_y, ball_x, ball_y, brick_matrix, score, lives, enable)
   begin
      vga_r <= "0000";
      vga_g <= "0000";
      vga_b <= "0000";
      if video = '1' and enable = '1' then
         ...
         if count_v >= paddle1_y and count_v < paddle1_y + PAD_HEIGHT and count_h >= paddle1_x and count_h < paddle1_x + PAD_WIDTH then
            vga_r <= "1111";  -- White paddle
            vga_g <= "1111";
            vga_b <= "1111";
         -- Top paddle (Player 2)
         elsif count_v >= paddle2_y and count_v < paddle2_y + PAD_HEIGHT and count_h >= paddle2_x and count_h < paddle2_x + PAD_WIDTH then
            vga_r <= "1111";  -- White paddle
            vga_g <= "1111";
            vga_b <= "1111";
         -- Ball
            ...
            vga_g <= "1111";
            vga_b <= "0000";
         -- Bricks (8x4 grid, centered)
         else
            for i in 0 to 31 loop
               if brick_matrix(i) = '1' and
                  count_h >= (i mod 8) * (BRICK_WIDTH + 10) + 60 and count_h < (i mod 8) * (BRICK_WIDTH + 10) + 120 and
                  count_v >= (i/8) * (BRICK_HEIGHT + 10) + BRICK_START_Y and count_v < (i/8) * (BRICK_HEIGHT + 10) + BRICK_START_Y + BRICK_HEIGHT then
                     case i/8 is
         ...
         -- Lives display (shared, top-left corner)
         if count_h < 48 and count_v < 25 then
            vga_r <= "1000";  -- Red placeholder for score/lives
