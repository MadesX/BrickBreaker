...
entity tb_screens is
  -- test bench for screens
end tb_screens;

architecture arc_tb_screens of tb_screens is
   component screens
      port (
         mode       : in  std_logic;
         video      : in  std_logic;
   ...
   -- signals
   signal mode       : std_logic := '0'; -- display mode
   ...
   signal enable     : std_logic := '0'; -- enable screen
...
begin
   -- screens instantiation (named association)
   eut: screens
      port map (
         mode       => mode,
         video      => video,
   ...
   begin
      -- initialize inputs
      mode <= '0'; enable <= '0'; left <= '0'; right <= '0'; sel <= '0'; clrn <= '1';
      count_h <= (others => '0'); count_v <= (others => '0');
      wait for 40 ns; -- wait for end of async reset

      ----------------------------------------- vector 1: enable screen, mode 0, check initial stage
      report "enabling screen with mode 0 and checking initial stage";
      mode <= '0'; enable <= '1';
      sim_vga_frame;
      ...
      ----------------------------------------- vector 2: press left button, mode 0
      report "pressing left button with mode 0";
      left <= '1';
      wait for clk_50_period * 10;
      left <= '0';
      sim_vga_frame;
      assert next_stage /= "00" report "stage did not change after left #2" severity error;
      wait for clk_25_period * 10;

      ----------------------------------------- vector 3: press right and select, mode 1
      report "pressing right and select buttons with mode 1";
      mode <= '1';
      right <= '1';
      wait for clk_50_period * 10;
      right <= '0';
      ...
      sel <= '0';
      sim_vga_frame;
      assert next_stage /= "00" report "stage did not change after right/select #3" severity error;
      wait for clk_25_period * 10;
